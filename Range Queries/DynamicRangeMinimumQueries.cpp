...
int rangeMinimumQ(int st,int end, int i , int l , int r){
    if(r<st || l>end)return 1e9;
    if(l>=st && r<=end)return segTree[i];
    int m = l+(r-l)/2;
    int left = rangeMinimumQ(st,end,2*i+1,l,m);
    int right = rangeMinimumQ(st,end,2*i+2,m+1,r);
...
void solve() {
    ...
    for(int i=0;i<q;i++){
        ...
        if(a==1){
            ...
            // range sum
            cout<<rangeMinimumQ(b-1,c-1,0,0,n-1)<<endl;
        }
    }
